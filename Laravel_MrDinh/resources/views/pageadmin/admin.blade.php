  ...
  <table id="table_admin_product" class="table table-striped display">
    ...
    <tbody>
      ...
      <tr class="products-list-admin">
        ...
        <th><img src="image/product/{{$product->image}}" alt="image" style="width: 50px ; height: 50px; object-fit: cover;" /></th>
        <td>{{$product->name}}</td>
        ...
        <td>
          ...
          <form role="form" action="admin/delete/{{$product->id}}" method="post">
            @csrf
            <button name="edit" type="submit" class="btn btn-danger" onclick="return confirm('Bạn có chắc chắn muốn xóa sản phẩm này không?');" style="width:80px;">Delete</button>
          </form>
        </td>
      </tr>
      @endforeach
